...
function toggleCheckInDetails(checkIn, listItem, detailsButton) {
    ...
    const allDetails = document.querySelectorAll(".checkin-details");
    allDetails.forEach((detail) => detail.remove());

    const allButtons = document.querySelectorAll(".view-details-button");
    allButtons.forEach((button) => button.classList.remove("open"));

    // Criar os detalhes do Check-In
    ...
    // Adicionar evento de clique nas imagens clicáveis
...
// ****** SEPARAÇÃO ENTRE HISTÓRICO E REALIZAÇÃO DE CHECK-IN ******
...
async function handlePanelSelection(event) {
    ...
    const activeMedia = playlistDetails.items.flatMap(item => {
        ...
        // Caso o item contenha uma lista de mídias, retorne-a
        return item.items;
    });
